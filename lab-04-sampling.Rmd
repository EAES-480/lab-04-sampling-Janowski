...
df <- df %>%
  mutate(
    ...
    date = as.Date(___ - 1, origin = paste0(___, "-01-01")),

    # TODO: create a month column (numeric 1–12 or labeled months)
    month = month(___, label = TRUE, abbr = TRUE),
...
df %>% summarise(
  n = n(),
  n_missing = sum(is.na(____)),
  mean = mean(____, na.rm = TRUE),
  sd = sd(____, na.rm = TRUE)
)
```
...
ggplot(df, aes(x = ____)) +
  geom_histogram(bins = ___, alpha = 0.7) +
  theme_classic(base_size = 18) +
...
pop_mean <- mean(df____, na.rm = TRUE)
pop_sd   <- sd(df____, na.rm = TRUE)
...
samp_mean <- mean(samp____, na.rm = TRUE)
samp_sd   <- sd(samp____, na.rm = TRUE)
