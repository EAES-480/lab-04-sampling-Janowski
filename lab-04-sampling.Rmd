...
df <- read_csv("data/us-ams-simple.csv") %>%
  ...
  mutate(
    doy = if_else(doy == -9999, NA_integer_, doy)) %>%
  filter(!is.na(doy))
...
> I had to replace values of -9999 with NAs and then filter out those NAs in the data because it was giving me errors when trying to generate the plots. I did have to use Chat GPT for this part while troubleshooting.
...
response_var <- "gpp"   # replace "gpp" with your choice, e.g. "fc" or "le" or "ta"
...
> I chose GPP because it is a very common variable that EAES scienrtoist are trying to understand in ecosystem studies. I expect to see an increase in gpp during the summer, and a decrease during the winter. The day/night pattern should show an increase during the day and a decrease at night.
...
|----------|-------|-------------|----------------------------|
|GPP|µmolCO₂ m⁻² s⁻¹|Gross Primary Productivity|high in summer, low in winter|
|TA|°C|Air temperature|higher in summer, lower in winter|
|G|W/m²|Soil heat flux|higher in summer, lower in winter|
|P|mm|Precipitation|Higher in wet seasons, lower in dry seasons|
|PA|kPa|Atmospheric pressure|Lower in summer, higher in winter|
...
> I see a seasonal pattern in gpp. It is higher during the summer and lower during the winter. There is also a daily cycle, where gpp is higher during the day and lower during the night. Some outliers are more visible during spring and fall due to higher variablity in the temperature during these seasons.
...
> The distribution of gpp is slightly right-skewed, but with an overall unimodal distribution, centered around low, positive gpp values. The spread is wide with slighly more values toward the positve end of the graph when compared to the negative end.
...
> The month variable explains the variability in gpp more than the day/night variable. There is a clearly visible variation in gpp by monthmwith changes in the average and range of expected values. The day/night variable shows a present, but slight variation that is not as extreme as the by month variation. This is because seasonality plays a larger role in gpp.
...
pop_mean <- mean(df$gpp, na.rm = TRUE)
pop_sd   <- sd(df$gpp, na.rm = TRUE)
...
samp_mean <- mean(samp$gpp, na.rm = TRUE)
samp_sd   <- sd(samp$gpp, na.rm = TRUE)
...
> The one sample estimate was very close to the expected average and standard deviation found across the population. It had a slightly lower mean and sd when compared to overall population, but, regardless, it was still very close. No, this doesnt surprise me becasue our data showed a very high clustering around a unimodal point. This means that we are more likely to generate a value close to the average of the population.
...
> Sampling distribution is centered around the population mean. The variability of the sample means decreases as we increase (n_samp). Bigger sample sizes show more accurate estimates of the population mean.
...
strat_mean <- mean(samp_strat$gpp, na.rm = TRUE)
strat_sd   <- sd(samp_strat$gpp, na.rm = TRUE)
...
> SRS provided a better approximation of the population mean and standard deviation compared to stratified sampling by month. The mean for the SRS sample was closer to the population mean, and the standard deviation was also smaller for SRS compared to the stratified sample. In this case, stratification might not have helped because of months not being good choices for groups in this case or high variability within the month groupings.
...
> Seasonality matters for sampling because variables in EAES often change over time, such as with the seasons in out case. Ignoring latent grouping variables can give biased estimates or incorrectly weighted estimates. In EAES field studies, stratification is essential when variables show seasonal changes with low variability. We saw in our data that stratified sampling was actually less acurrate, so its impirtant to set up the stratifications properly to avoid errors or high variability. It could also be difficult to have equal samples across all of the time periods (seasons/days/nights). Maybe it is easier, cheaper, or more reliable to do random sampling at times.
...
> It did much worse than SRS for predicting the mean, expecially under certain intervals. 48 and 96 yielded vastly incorrrect results in the mean, but 24 was much closer to accurate. Although, it was still underestimating when compared to SRS and the population mean. Higher interval values did yield a lower, more accurate SD. Daily cycles are interacting with the values, expecially if the interval is high and introducing times of low gpp at night into the mean.
...
> Cluster sampling can result in higher variance because it selects days as clusters, and all times within the days are included in the sample. This means that the sample may have points that are similar to each other another and this will reduce the variability. If clusterings of days are used here, the sample may not show the variability of the entire population.
...
> Quasi-continuous sampling preserves the seasonal pattern in gpp, but shows less of the the daily patterns because it samples at standard intervals. This means that we cant see the daily variation as well as other models. In EAES studies, quasi-continuous sampling is preferable when measurements are needed at consitent intervals. For example, in cases where we want to see chnages at specific times or days across a longer time period.
...
> Quasi-continuous sampling was the best estimate of the population mean, as the mean almost exactly matched the population mean. For SD, it performed close to the population as well. The quasi-continuous approach helps show the seasonal and some of the diurnal cycles. It also lowers sampling bias from clustered sampling. In a real EAES study, a hybrid strategy of quasi-continuous sampling for seeing consistent patterns in the data, combined with stratified sampling to see seasonal chnages, would provide a good estimate that captures the needed  data.
